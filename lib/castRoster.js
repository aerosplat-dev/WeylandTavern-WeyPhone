 ...
 * @param {{
 *   weybooruCharacters: Record<string, {bot?: string, image?: string}>,
 *   weylandEntries: Array<{comment?: string, content?: string}>,
 *   charPerKeys: string[],
 *   excludedEntryNames?: string[],
 *   manualFullBotOnlyNames?: string[],
 * }} options
 * @returns {Array<{fullName: string, entryName: string, macroKey: string | null, hasFullBot: boolean, hasSubbot: boolean, portraitSlug: string}>}
 *   sorted alphabetically by fullName (the name actually shown in the New Message contact list),
 *   so manualFullBotOnlyNames entries (Loona, Kressa) land in their natural alphabetical place
 *   instead of always trailing at the end of the list.
 */
export function buildCastRoster({ weybooruCharacters, weylandEntries, charPerKeys, excludedEntryNames = [], manualFullBotOnlyNames = [] }) {
    ...
    for (const [fullName, data] of Object.entries(weybooruCharacters)) {
        ...
        roster.push({
            ...
            hasSubbot: true,
            portraitSlug: resolvePortraitSlug(data, tokens),
        });
    }
    for (const name of manualFullBotOnlyNames) {
        ...
        roster.push({
            ...
            hasSubbot: false,
            portraitSlug: tokens[0].toLowerCase(),
        });
    }
...
/**
 * cast.weybooru.com's own `image` field is the authoritative portrait filename (sans extension) —
 * it does NOT always match a naive lowercased-first-token guess. Two confirmed real mismatches:
 * "Garret Sullivan" (note: misspelled on the cast page itself, missing a second "t") is served as
 * `garrett.jpg`, and "Margaret Sullivan" is served under her nickname, `marge.jpg`. Preferring
 * `data.image` when present avoids re-deriving (and re-guessing wrong) a slug we're already handed.
 * Falls back to the old first-token guess only if `image` is missing/blank, which no real character
 * currently exercises (confirmed live: every entry in cast.weybooru.com's catalog has a non-empty
 * `image` field) but is kept as a defensive fallback rather than a hard requirement.
 * @param {{image?: string}} data
 * @param {string[]} tokens output of splitFullName, in original order
 * @returns {string}
 */
function resolvePortraitSlug(data, tokens) {
    if (typeof data.image === 'string' && data.image.trim()) return data.image.trim().toLowerCase();
