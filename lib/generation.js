...
/**
 * Resolves which model id (if any) to send as ConnectionManagerRequestService's overridePayload.
 * An explicit `settings.modelId` (set via WeyPhone's own extension settings panel — see
 * settings.html) is a deliberate user choice to generate WeyPhone content with a specific model,
 * and always wins. Falling back to `liveModel` when no override is set preserves the existing
 * behavior of tracking the main roleplay chat's active model (see generateReply/
 * runFlavorAppGeneration's own call sites) rather than a Connection Profile's stale saved model.
 * Returns `null` (not `''`) when neither is set, so callers can build `overridePayload` with a
 * simple truthiness check exactly like the pre-existing `liveModel` pattern already did.
 * @param {{modelId?: string}} settings WeyPhone settings
 * @param {string | null | undefined} liveModel
 * @returns {string | null}
 */
export function resolveModelOverride(settings, liveModel) {
    const explicit = settings.modelId?.trim();
    if (explicit) return explicit;
    return liveModel || null;
}
