 ...
 * @param {{cautionKey: string, positionInPrompt: number, positionInChat: number, positionNone: number}} positions
 * @returns {{ops: Array<{key: string, content: string, position: number, depth: number}>, nextKeys: Set<string>}}
 */
export function planTetherExtensionPromptOps(plan, previousKeys, { cautionKey, positionInPrompt, positionInChat, positionNone }) {
    const ops = [];
    const nextKeys = new Set();
    if (plan.cautionBlock) {
        ops.push({ key: cautionKey, content: plan.cautionBlock, position: positionInPrompt, depth: 0 });
        nextKeys.add(cautionKey);
    }
    for (const group of plan.groups) {
        ops.push({ key: group.key, content: group.content, position: positionInChat, depth: group.depth });
        nextKeys.add(group.key);
    }
    for (const prev of previousKeys) {
        if (!nextKeys.has(prev)) {
            ops.push({ key: prev, content: '', position: positionNone, depth: 0 });
        }
    }
    return { ops, nextKeys };
}
