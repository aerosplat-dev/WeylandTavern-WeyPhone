 ...
 * @param {Record<string, {primaryUrl: string|null, fallbackUrl: string|null, initial: string|null}>} portraitMap keyed by summary.participants.join(', ') — matches how index.js's renderMessagesScreenNow/renderThreadsScreenNow build it (see lib/storage.js's post-Task-5 participants schema; group-specific per-participant rendering is Task 9's job)
 */
export function renderMessagesScreen(container, summaries, formatRelativeTime, portraitMap) {
    ...
    container.innerHTML = summaries.map(summary => {
        const displayName = summary.participants.join(', ');
        return `
<div class="wp-list-item wp-conversation-item" data-id="${escapeHtml(summary.id)}">
    ${avatarMarkup(portraitMap[displayName])}
    <div class="wp-list-item-main">
