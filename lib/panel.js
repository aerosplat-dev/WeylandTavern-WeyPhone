...
/**
 * Best-effort decoration for a Discord item's text — flags @luckypaww's posts as a system/server
 * notice. Channel identity is NOT extracted from the item text: lib/phoneAppPrompts.js's discord
 * prompt puts each channel in its own "## #channel-name" section header (rendered as this
 * section's title, see renderPhoneAppScreen below), so the message body never needs to name its
 * own channel. Gracefully falls through to plain text if the pattern isn't present — never assumes
 * the model's output matches perfectly.
 * @param {string} text
 * @returns {{isServerPost: boolean, text: string}}
 */
function decorateDiscordItem(text) {
    const isServerPost = /^\[?@luckypaww\]?/.test(text);
    return { isServerPost, text };
}
