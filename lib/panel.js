...
/**
 * A single row in the plain scrollable contact list (New Message screen). Outside group mode this
 * is a plain clickable row (data-contact-for) that starts a solo conversation immediately. In group
 * mode every row gets a checkbox; rows for contacts with no subbot (`hasSubbot: false` — Loona,
 * Kressa) render disabled/greyed since they can't participate in a group thread.
 * @param {{entryName: string, portraitSlug: string, hasSubbot: boolean}} contact
...
<div class="${classes.join(' ')}" data-contact-for="${escapeHtml(contact.entryName)}">
    ...
    <div class="wp-list-item-main">
        <div class="wp-list-item-title">${escapeHtml(contact.entryName)}</div>
    </div>
    ${checkbox}
</div>`;
}
 ...
 * (full-bots + subbots, plus manually-appended full-bot-only contacts). Outside group mode,
 * clicking a row starts a solo conversation immediately. In group mode (`groupMode: true`), rows
 * become checkboxes and the header shows Cancel/Done instead of the Group Chat button.
 * @param {HTMLElement} container #wp-screen-body
 * @param {{
 *   roster: Array<{entryName: string, portraitSlug: string, hasSubbot: boolean}>,
 *   groupMode: boolean,
 *   selectedEntryNames: string[],
 * }} state
 */
export function renderContactsScreen(container, { roster, groupMode, selectedEntryNames }) {
    const selectedSet = new Set(selectedEntryNames);
    const header = groupMode
        ? `
<div id="wp-contacts-header">
    <button id="wp-group-cancel-button" class="menu_button">Cancel</button>
    <button id="wp-group-done-button" class="menu_button" ${selectedSet.size === 0 ? 'disabled' : ''}>Done (${selectedSet.size})</button>
</div>`
        : `
<div id="wp-contacts-header">
    <button id="wp-group-chat-button" class="menu_button">Group Chat</button>
</div>`;

    const listMarkup = roster.length
        ? roster.map(contact => contactRowMarkup(contact, groupMode, selectedSet.has(contact.entryName))).join('')
        : '<div class="wp-empty-state">No contacts available yet.</div>';
