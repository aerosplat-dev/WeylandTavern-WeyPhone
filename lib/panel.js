...
/**
 * Renders a weybooru-primary image with an inline onerror fallback to the local SillyTavern
 * avatar URL — the only reliable way to detect a failed external-CDN image load from plain HTML
 * without a real pre-flight network round-trip. `this.onerror=null` before swapping prevents an
 * infinite loop if the fallback URL itself also fails to load.
...
function avatarMarkup(portrait) {
    if (portrait && portrait.primaryUrl) {
        const fallbackAttr = portrait.fallbackUrl
            ? ` onerror="this.onerror=null;this.src='${escapeHtml(portrait.fallbackUrl)}'"`
            : '';
...
const APP_ICONS = {
    ...
    aethel: `${ASSET_BASE_URL}/weyphone_aethel.png`,
};
...
export function renderAppGridScreen(container, { flavorAppsEnabled }) {
    ...
    // Aethel's tile is deliberately NEVER greyed out, regardless of main-roleplay state or whether
    // the Aethel character card is actually installed — openAethelConversation() already handles
    // the "not installed" case with a clear error toast at click-time instead.
    container.innerHTML = `
<div class="wp-app-grid">
    ...
    <div class="wp-app-tile" data-app="aethel">
        <div class="wp-app-icon"><img src="${APP_ICONS.aethel}" alt="" /></div>
        <div class="wp-app-label">Aethel: The Game</div>
    </div>
...
const DISCORD_CHANNEL_RE = /in #([\w-]+):/;
const YIKYAK_VOTE_RE = /\s*\+(\d+)\s*$/;

/**
 * Best-effort decoration for a Discord item's text — extracts a "#channel" mention (if present)
 * into a separate pill, and flags @luckypaww's posts as a system/server notice. This is NOT a new
 * structured parser — Discord's underlying data is still the generic {sections} shape; this only
 * changes how the SAME text is decorated at render time, matching the item-text conventions
 * already established in lib/phoneAppPrompts.js's discord prompt ("[@handle] in #channel: text").
 * Gracefully falls through to plain text if the pattern isn't present — never assumes the model's
 * output matches perfectly.
 * @param {string} text
 * @returns {{channelTag: string|null, isServerPost: boolean, text: string}}
 */
function decorateDiscordItem(text) {
    const isServerPost = /^\[?@luckypaww\]?/.test(text);
    const channelMatch = text.match(DISCORD_CHANNEL_RE);
    // Strip the matched "in #channel:" substring out of the body once it's been pulled into its
    // own pill, so the channel name doesn't render twice (mirrors decorateYikYakItem stripping
    // its own extracted "+NN" suffix out of the body).
    const strippedText = channelMatch ? text.replace(DISCORD_CHANNEL_RE, '').replace(/\s{2,}/g, ' ').trim() : text;
    return { channelTag: channelMatch ? channelMatch[1] : null, isServerPost, text: strippedText };
}
...
/**
 * @param {HTMLElement} container #wp-screen-body
 * @param {{appLabel: string, entry: {content: {sections: Array<{title: string, items: Array<{text: string, timestamp?: string}>}>}, generatedAt: number} | undefined, isGenerating: boolean, formatRelativeTime: (epochMs: number) => string, showFollowingLink?: boolean}} state
 */
export function renderPhoneAppScreen(container, { appLabel, entry, isGenerating, formatRelativeTime, showFollowingLink = false }) {
    ...
    const followingLink = showFollowingLink
        ? '<button id="wp-twitter-following-link" class="menu_button wp-secondary-button">Following →</button>'
        : '';
...
<div id="wp-phone-app-actions">${refreshButton}</div>`;
    ...
    const sectionsHtml = entry.content.sections.map(section => {
        // Yik Yak's own prompt only ever produces one flat "YIK YAK" section, which is entirely
        // redundant with the panel's own header title ("Yik Yak") — suppress the in-content
        // section title just for this app rather than showing the same name twice. Chronicle's
        // WEYLAND ALERTS/HEADLINES titles and Discord's own section title(s) are NOT redundant
        // with their panel headers ("The Chronicle"/"Discord"), so they stay visible as before.
        const sectionTitleHtml = appLabel === 'Yik Yak'
            ? ''
...
<div class="wp-phone-app-section">
    ...
    ${section.items.map(item => {
        if (appLabel === 'Discord') {
            const decorated = decorateDiscordItem(item.text);
            const serverClass = decorated.isServerPost ? ' wp-discord-server-post' : '';
            const channelPill = decorated.channelTag ? `<span class="wp-discord-channel-pill">#${escapeHtml(decorated.channelTag)}</span>` : '';
            return `
...
<div class="wp-twitter-post">
    ${avatarMarkup(portraitMap[post.authorName])}
    <div class="wp-twitter-post-main">
        <div class="wp-twitter-post-header">
            <span class="wp-twitter-post-name">${escapeHtml(post.authorName)}</span>
 ...
 * (e.g. to frame/set up context for Aethel's dedicated app before ever sending anything). Only the
...
/**
 * Hides the tethered-mode toggle entirely (not just disables it) — used for Aethel's dedicated
 * app, which is permanently untethered and shouldn't show a toggle that implies a choice exists.
 * @param {HTMLElement} labelEl #wp-mode-toggle
 * @param {boolean} visible
 */
export function setModeToggleVisible(labelEl, visible) {
