 ...
 *   When isGroup is true, each assistant message's name/avatar render iMessage-style: the speaker's
 *   name appears above only the FIRST message in a same-speaker run, and their avatar appears
 *   beside only the LAST message in that run — resetting whenever the speaker changes or a user
 *   message interrupts the run. When isGroup is false/omitted, no name/avatar markup is ever added
 *   (matching a solo conversation's existing always-right-justified-user /
 *   always-left-justified-character convention) — note this is a visual/behavioral guarantee only,
 *   not a literal DOM-structure one: every message row (group or solo) is wrapped in a
 *   `.wp-message-row` div added by this same milestone, whose flex properties are a verbatim copy
 *   of `.wp-message`'s own pre-existing ones, so bubbles still shrink-wrap to content identically.
 */
export function renderMessages(container, messages, editingIndex = -1, isTyping = false, selectState = {}, groupState = { isGroup: false, portraitMap: {} }) {
    ...
    messages.forEach((message, index) => {
        ...
        if (isGroup && message.role === 'assistant') {
            ...
            bubble.classList.add('wp-message-grouped');
        }
