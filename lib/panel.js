...
import { formatParticipantNames, resolveThreadDisplayName } from './participants.js';

export function createPanelMarkup() {
    return `
<div id="wp-toggle-button" title="WeyPhone">
    <i class="fa-solid fa-mobile-screen"></i>
    <span id="wp-toggle-unread-badge" class="wp-unread-badge" hidden></span>
</div>
...
/**
 * Renders 2-4 overlapping avatars as a "deck of cards" for a group conversation — no single
 * weybooru portrait applies to a group, so this composites each participant's own portrait
 * (already resolvable per-name via the existing portraitMap/avatarMarkup convention) into one
 * stacked visual. Caps at 4 visible cards even if more participants exist (a group's avatar is a
 * small header/list icon, not a full participant roster) — the display NAME (formatParticipantNames)
 * is what conveys "and N more", not the avatar stack itself.
 * @param {Array<{primaryUrl: string|null, fallbackUrl: string|null, initial: string|null}>} portraits in participant order
 * @returns {string}
 */
function avatarStackMarkup(portraits) {
    const visible = portraits.slice(0, 4);
    const cards = visible.map((portrait, index) => {
        // Reuses avatarMarkup's own per-avatar HTML (fallback-swap included) — just wraps it in a
        // positioned card div. index 0 renders first (bottom of the stack, painted over by later
        // ones) and gets the least rotation; later cards rotate further and paint on top.
        return `<div class="wp-avatar-stack-card" style="--wp-stack-index: ${index};">${avatarMarkup(portrait)}</div>`;
    }).join('');
    return `<div class="wp-avatar-stack" style="--wp-stack-count: ${visible.length};">${cards}</div>`;
}

/**
 * Renders EITHER a single avatar (solo conversation) or a stacked deck (group conversation, 2+
 * portraits) into a container — the one shared entry point index.js should call instead of
 * choosing between avatarMarkup/avatarStackMarkup itself at every call site.
 * @param {{primaryUrl: string|null, fallbackUrl: string|null, initial: string|null} | Array<{primaryUrl: string|null, fallbackUrl: string|null, initial: string|null}>} portraitOrPortraits
 * @returns {string}
 */
export function anyAvatarMarkup(portraitOrPortraits) {
    if (Array.isArray(portraitOrPortraits)) {
        return portraitOrPortraits.length > 1 ? avatarStackMarkup(portraitOrPortraits) : avatarMarkup(portraitOrPortraits[0]);
    }
    return avatarMarkup(portraitOrPortraits);
}
...
/**
 * Sets (or clears) the panel header's avatar. Accepts either a single portrait (solo conversation)
 * or an array of portraits (group conversation, rendered as a stacked deck via anyAvatarMarkup) —
 * pass null/undefined/an empty array to clear.
 * @param {HTMLElement} container #wp-panel-avatar
 * @param {{primaryUrl: string|null, fallbackUrl: string|null, initial: string|null} | Array<{primaryUrl: string|null, fallbackUrl: string|null, initial: string|null}>} [portraitOrPortraits] pass null/undefined/[] to clear
 */
export function renderPanelAvatar(container, portraitOrPortraits) {
    const isEmpty = !portraitOrPortraits || (Array.isArray(portraitOrPortraits) && portraitOrPortraits.length === 0);
    container.innerHTML = isEmpty ? '' : anyAvatarMarkup(portraitOrPortraits);
}
...
/**
 * @param {HTMLElement} container #wp-screen-body
 * @param {{flavorAppsEnabled: boolean, messagesUnreadTotal?: number}} state
 */
export function renderAppGridScreen(container, { flavorAppsEnabled, messagesUnreadTotal = 0 }) {
    const disabledClass = flavorAppsEnabled ? '' : ' wp-app-tile-disabled';
    const messagesBadge = messagesUnreadTotal > 0
        ? `<span id="wp-messages-tile-unread-badge" class="wp-unread-badge">${messagesUnreadTotal > 99 ? '99+' : messagesUnreadTotal}</span>`
        : '<span id="wp-messages-tile-unread-badge" class="wp-unread-badge" hidden></span>';
    container.innerHTML = `
<div class="wp-app-grid">
    ...
    <div class="wp-app-tile" data-app="messages">
        <div class="wp-app-icon"><img src="${APP_ICONS.messages}" alt="" />${messagesBadge}</div>
 ...
 */
export function decorateDiscordItem(item) {
 ...
 */
export function decorateYikYakItem(text) {
...
/**
 * @param {HTMLElement} container #wp-screen-body
 * @param {Array<{id: string, participants: string[], lastMessageSnippet: string, lastActive: number, isTyping?: boolean, unreadCount?: number, tethered?: boolean}>} summaries
 * @param {(epochMs: number) => string} formatRelativeTime
 * @param {Record<string, {primaryUrl: string|null, fallbackUrl: string|null, initial: string|null}>} portraitMap keyed by INDIVIDUAL participant name (matches buildPortraitMap's real contract) — each summary's participants are looked up one at a time and rendered as a stacked avatar deck for groups.
 */
export function renderMessagesScreen(container, summaries, formatRelativeTime, portraitMap) {
    ...
    container.innerHTML = summaries.map(summary => {
        const displayName = resolveThreadDisplayName(summary);
        const portraits = summary.participants.map(name => portraitMap[name]);
        const avatar = summary.participants.length > 1 ? portraits : portraits[0];
        return `
<div class="wp-list-item wp-conversation-item" data-id="${escapeHtml(summary.id)}">
    ${anyAvatarMarkup(avatar)}
    <div class="wp-list-item-main">
        <div class="wp-list-item-title">${escapeHtml(displayName)}${summary.tethered ? ' <i class="fa-solid fa-link wp-tether-chain" title="Tethered to the active roleplay"></i>' : ''}</div>
    ...
    <div class="wp-list-item-meta">
        ...
        ${(summary.unreadCount ?? 0) > 0 ? `<span class="wp-unread-badge wp-list-item-unread">${summary.unreadCount > 99 ? '99+' : summary.unreadCount}</span>` : ''}
...
/**
 * A single row in the plain scrollable contact list (New Message screen). Displays the contact's
 * FULL name (as scraped from cast.weybooru.com) for legibility/searchability — clicking the row
 * still starts/joins a thread keyed by its shorter subbot/entry name (`data-contact-for`, read by
 * index.js's click handlers), never the displayed text. Outside group mode this is a plain
 * clickable row that starts a solo conversation immediately. In group mode every row gets a
 * checkbox; rows for contacts with no subbot (`hasSubbot: false` — Loona, Kressa) render
 * disabled/greyed since they can't participate in a group thread.
 * @param {{entryName: string, fullName: string, portraitSlug: string, hasSubbot: boolean}} contact
 * @param {boolean} groupMode
 * @param {boolean} selected
 * @returns {string}
 */
function contactRowMarkup(contact, groupMode, selected) {
    const disabled = groupMode && !contact.hasSubbot;
    const checkbox = groupMode
        ? `<input type="checkbox" class="wp-contact-checkbox" ${selected ? 'checked' : ''} ${disabled ? 'disabled' : ''} />`
        : '';
    const classes = ['wp-list-item', 'wp-contact-row'];
    if (disabled) classes.push('wp-contact-row-disabled');
    return `
<div class="${classes.join(' ')}" data-contact-for="${escapeHtml(contact.entryName)}">
    <img class="wp-avatar" src="${escapeHtml(WEYBOORU_PORTRAIT_BASE_URL)}/${escapeHtml(contact.portraitSlug)}.jpg" alt="" />
    <div class="wp-list-item-main">
        <div class="wp-list-item-title">${escapeHtml(contact.fullName)}</div>
    </div>
    ${checkbox}
</div>`;
}
...
/**
 * The "New Message" screen: a plain scrollable contact list sourced from the cast roster
 * (full-bots + subbots, plus manually-appended full-bot-only contacts), each row showing the
 * contact's full weybooru name. A single header row holds a small selection-mode toggle button
 * (checkmark-in-a-box icon, press to enter/exit group-selection mode) to its left of a search
 * input that live-filters the list by full-name substring; in selection mode the search input
 * shrinks to make room for a send-icon button (starts the group thread with whatever's selected,
 * disabled while nothing is selected).
 * @param {HTMLElement} container #wp-screen-body
 * @param {{
 *   roster: Array<{entryName: string, fullName: string, portraitSlug: string, hasSubbot: boolean}>,
 *   groupMode: boolean,
 *   selectedEntryNames: string[],
 *   searchQuery: string,
 * }} state
 */
export function renderContactsScreen(container, { roster, groupMode, selectedEntryNames, searchQuery }) {
    const selectedSet = new Set(selectedEntryNames);
    const trimmedQuery = searchQuery.trim().toLowerCase();
    const filteredRoster = trimmedQuery
        ? roster.filter(contact => contact.fullName.toLowerCase().includes(trimmedQuery))
        : roster;

    const toggleTitle = groupMode ? 'Cancel selection' : 'Select multiple contacts';
    const sendButton = groupMode
        ? `<button id="wp-group-send-button" class="wp-contacts-icon-btn" ${selectedSet.size === 0 ? 'disabled' : ''} title="Start group chat"><i class="fa-solid fa-paper-plane"></i></button>`
        : '';
    const header = `
<div id="wp-contacts-header">
    <button id="wp-group-toggle-button" class="wp-contacts-icon-btn${groupMode ? ' wp-active' : ''}" title="${toggleTitle}"><i class="fa-solid fa-square-check"></i></button>
    <input type="text" id="wp-contact-search-input" placeholder="Search" value="${escapeHtml(searchQuery)}" autocomplete="off" />
    ${sendButton}
</div>`;

    const listMarkup = filteredRoster.length
        ? filteredRoster.map(contact => contactRowMarkup(contact, groupMode, selectedSet.has(contact.entryName))).join('')
        : `<div class="wp-empty-state">${trimmedQuery ? 'No matching contacts.' : 'No contacts available yet.'}</div>`;

    container.innerHTML = `${header}<div id="wp-contact-list">${listMarkup}</div>`;
}
...
<div id="wp-input-row">
    <div id="wp-regenerate-wrapper">
        ...
        <div id="wp-regenerate-menu" class="wp-popup-menu" hidden>
            ...
            <button type="button" class="wp-popup-menu-item" data-action="import-scenario">Import from Scenario</button>
            <button type="button" class="wp-popup-menu-item" data-action="set-nickname">Set Nickname</button>
            <button type="button" class="wp-popup-menu-item" data-action="switch-threads">Switch Threads</button>
            <button type="button" class="wp-popup-menu-item" data-action="rename-thread">Rename Thread</button>
 ...
 * individual Regenerate/Delete Messages/Import from Scenario items are conditionally disabled,
 * since those genuinely have nothing to act on (or are gated behind a setting) — Memory is never
 * disabled.
 * @param {HTMLElement} menuEl #wp-regenerate-menu
 * @param {{canRegenerate: boolean, hasMessages: boolean, canImportScenario: boolean}} state
 */
export function setRegenerateMenuItemsEnabled(menuEl, { canRegenerate, hasMessages, canImportScenario }) {
    const regenerateItem = menuEl.querySelector('[data-action="regenerate"]');
    const selectItem = menuEl.querySelector('[data-action="select"]');
    const importItem = menuEl.querySelector('[data-action="import-scenario"]');
    if (regenerateItem) regenerateItem.disabled = !canRegenerate;
    if (selectItem) selectItem.disabled = !hasMessages;
    if (importItem) importItem.disabled = !canImportScenario;
}
...
/**
 * Deterministic per-speaker accent color for a group message's name label — the same character
 * always lands on the same hue across renders/sessions (a stable string hash into HSL), used ONLY
 * for that small label's text color, never the bubble itself. Keeping every character's bubble
 * the same quiet dark tone (matching this app's existing two-tone convention: one shade for the
 * user, one for everyone else) avoids turning a group thread into a rainbow of competing bubble
 * colors, while still giving each name its own consistent, at-a-glance identity color the way
 * Discord/Slack usernames do.
 * @param {string} name
 * @returns {string}
 */
export function speakerAccentColor(name) {
    let hash = 0;
 ...
 * @param {Array<{role: string, content: string, speaker?: string}>} messages
 * @param {number} [editingIndex]
 * @param {boolean} [isTyping]
 * @param {{active?: boolean, selectedIndices?: Set<number>}} [selectState]
 * @param {{isGroup: boolean, portraitMap: Record<string, {primaryUrl: string|null, fallbackUrl: string|null, initial: string|null}>}} [groupState]
 *   When isGroup is true, each assistant message's name/avatar render iMessage-style, OUTSIDE the
 *   bubble itself: the speaker's name appears in small accent-colored text ABOVE only the FIRST
 *   message in a same-speaker run, and their avatar appears BESIDE (to the left of) only the LAST
 *   message in that run — resetting whenever the speaker changes or a user message interrupts the
 *   run. Each message in a run also gets a slightly tightened corner on the side facing its
 *   neighbor (`.wp-run-not-first`/`.wp-run-not-last`), the same "joined" bubble treatment iMessage
 *   uses, so a multi-message run reads as one continuous thought instead of separate stacked
 *   bubbles. When isGroup is false/omitted, no name/avatar/run-corner markup is ever added
 *   (matching a solo conversation's existing always-right-justified-user /
 *   always-left-justified-character convention). The default (non-select, non-editing) render
 *   path wraps the bubble in a `.wp-message-item` + `.wp-message-row` pair so the name label and
 *   avatar/spacer sit as true SIBLINGS of the bubble, not children of it — the bubble's own
 *   padding/background/border-radius therefore never wraps around them.
 */
export function renderMessages(container, messages, editingIndex = -1, isTyping = false, selectState = {}, groupState = { isGroup: false, portraitMap: {} }) {
    const { active: selectActive = false, selectedIndices = new Set() } = selectState;
    const { isGroup, portraitMap } = groupState;
    container.innerHTML = '';
    messages.forEach((message, index) => {
        ...
        if (isGroup && message.role === 'assistant') {
            ...
            showName = !prev || prev.role !== 'assistant' || prev.speaker !== message.speaker;
            showAvatar = !next || next.role !== 'assistant' || next.speaker !== message.speaker;
            if (!showName) bubble.classList.add('wp-run-not-first');
            if (!showAvatar) bubble.classList.add('wp-run-not-last');
        }
...
<div class="wp-message-edit-controls">
    ...
    <button class="wp-message-edit-cancel" title="Cancel"><i class="fa-solid fa-xmark"></i></button>
        ...
        textSpan.textContent = message.content;
        bubble.appendChild(textSpan);
        // Only the user's own messages are editable — a character's reply can only be changed by
        // regenerating it (see the Regenerate control), not hand-edited in place, so no edit
        // button is rendered for wp-char bubbles at all.
        ...
        // The bubble above holds ONLY the message content — name label and avatar/spacer are
        // built here as its SIBLINGS inside `item`/`row`, never appended into `bubble` itself, so
        // the bubble's own background/padding/border-radius never encloses them.
        const item = document.createElement('div');
        item.className = `wp-message-item ${message.role === 'user' ? 'wp-align-user' : 'wp-align-char'}`;
        if (showName) {
            const nameLabel = document.createElement('div');
            nameLabel.className = 'wp-message-speaker-name';
            nameLabel.textContent = message.speaker ?? '';
            nameLabel.style.color = speakerAccentColor(message.speaker ?? '');
            item.appendChild(nameLabel);
        }
        const row = document.createElement('div');
        row.className = 'wp-message-row';
        if (showAvatar) {
            const portrait = portraitMap[message.speaker] ?? null;
            const avatarWrapper = document.createElement('div');
            avatarWrapper.className = 'wp-message-avatar-slot';
            avatarWrapper.innerHTML = portrait && portrait.primaryUrl
                ? `<img class="wp-avatar" src="${escapeHtml(portrait.primaryUrl)}" alt="" />`
                : `<div class="wp-avatar wp-avatar-fallback">${escapeHtml(portrait && portrait.initial ? portrait.initial : '?')}</div>`;
            row.appendChild(avatarWrapper);
        } else if (isGroup && message.role === 'assistant') {
            const spacer = document.createElement('div');
            spacer.className = 'wp-message-avatar-slot';
