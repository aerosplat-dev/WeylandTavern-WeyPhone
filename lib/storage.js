 ...
 * @param {{isDedicatedApp?: string}} [options] tag this conversation as belonging to a dedicated
 *   Home-grid app (e.g. 'aethel') rather than a normal user-created conversation — excludes it from
 ...
 * @returns {Conversation | undefined}
 */
export function setTetheredSettings(settings, id, { tethered, tetheredHistoryCap } = {}) {
    const conversation = getConversation(settings, id);
    if (!conversation) return undefined;
...
export function getAllConversationSummaries(settings) {
    return Object.values(settings.conversations)
        .filter(conversation => !conversation.isDedicatedApp)
        .map(conversation => ({
            id: conversation.id,
 ...
 * every character including Aethel. isDedicatedApp only controls Messages-list visibility (see
 ...
 * show ALL of their threads, dedicated-app or not (for Aethel specifically, EVERY one of her
...
export function getThreadsFor(settings, charName) {
    return Object.values(settings.conversations)
        .filter(conversation => conversation.charName === charName)
        .map(conversation => ({
            id: conversation.id,
            charName: conversation.charName,
            lastMessageSnippet: conversation.messages.length
...
/**
 * Aethel's Home-tile entry point: resumes her most recently active thread (via the same
 * charName-based lookup every character uses — see findMostRecentThread), or creates a new one
 * tagged isDedicatedApp if she has no threads yet. This is a thin convenience wrapper, not a
 * separate mechanism — Start New Thread/Switch Threads/delete all work on Aethel's threads via the
 * exact same charName-based functions used for every other character.
 * @param {{conversations: Record<string, Conversation>}} settings
 * @param {string} charName
 * @param {string} appKey
 * @returns {Conversation}
