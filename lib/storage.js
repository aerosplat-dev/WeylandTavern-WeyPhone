...
/**
 * Finds the existing conversation tagged with this dedicated-app key (regardless of charName, in
 * case that ever needs to change), or creates a new one. Ensures a dedicated Home-grid app (e.g.
 * Aethel's) always resolves to exactly one persistent conversation, found-or-created idempotently.
 * @param {{conversations: Record<string, Conversation>}} settings
 * @param {string} charName
 * @param {string} appKey
 * @returns {Conversation}
...
export function findOrCreateDedicatedAppConversation(settings, charName, appKey) {
    const existing = Object.values(settings.conversations).find(c => c.isDedicatedApp === appKey);
