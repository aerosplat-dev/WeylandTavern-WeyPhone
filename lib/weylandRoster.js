...
/**
 * Characters with a real, in-fiction online username but who are explicitly NOT Weyland students —
 * confirmed by cross-referencing every discovered subbot's own personality prompt for an
 * online-username mention, then checking each hit against WEYLAND_ROSTER above. Kept in a SEPARATE
 * array (rather than folded into WEYLAND_ROSTER) because, per the operator's explicit direction,
 * neither would plausibly be present on the campus Discord server or the local Yik Yak stream —
 * only Twitter, where a wider, non-campus-bound audience makes sense for a visiting celebrity
 * (Bastet) and a local-but-unaffiliated parent (Navine). Consumers: ONLY twitterPrompts.js's
 * buildTwitterPrompt (feed mode) and index.js's Twitter Following list/profile-page wiring —
 * lib/phoneAppPrompts.js's Discord/Yik Yak prompts must keep importing WEYLAND_ROSTER alone.
 * @type {RosterCharacter[]}
 */
export const TWITTER_ONLY_ROSTER = [
    { name: "Navine", handle: "@unschoolingjourney_navine", bio: `- Middle aged catgirl housewife
- Sunny's overbearing mother, documents her love on social media
- Outspoken proponent of homeschooling
- Spreads misinformation with a gentle, worried tone` },
    { name: "Bastet", handle: "@RoyalBastet", bio: `- Egyptian catgirl, High Priestess of Bast
- Gorgeous, warm, playful, chaotic
- Livestreams videogames in scantily clad Kemetic attire
- Has millions of devout followers` },
];
